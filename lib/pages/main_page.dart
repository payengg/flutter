...
class _MainPageState extends State<MainPage> {
  ...
  List<Widget> _getPages(User user) => [
    ...
    AkunPage(user: user, controller: _scrollController),
  ];
      ...
      body: Stack(
        children: [
          ...
          AnimatedPositioned(
            ...
            left: 0,
            right: 0,
            child: _buildFloatingNavigationBar(selectedIndex), // Kirim selectedIndex
          ),
        ],
      ),
    );
  ...
  Widget _buildFloatingNavigationBar(int selectedIndex) { // Terima selectedIndex
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            _buildNavItem(icon: Icons.home, label: 'Beranda', index: 0, selectedIndex: selectedIndex),
            _buildNavItem(icon: Icons.chat_bubble_outline, label: 'Pesan', index: 1, selectedIndex: selectedIndex),
            _buildNavItem(icon: Icons.shopping_basket_outlined, label: 'Pesanan', index: 2, selectedIndex: selectedIndex),
            _buildNavItem(icon: Icons.favorite_border, label: 'Favorit', index: 3, selectedIndex: selectedIndex),
            _buildNavItem(icon: Icons.person_outline, label: 'Akun', index: 4, selectedIndex: selectedIndex),
          ],
        ),
      ),
    );
